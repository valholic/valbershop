        ...
        <div className="flex-grow p-2 lg:p-5 relative">
            {cartData.length !== 0 && 
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 p-8 items-center">
                    {cartData.map(cart => {
                        return <CartCard key={cart._id} img={cart.cart_image} name={cart.product_name} price={cart.amount * cart.price} pId={cart.product_id} cId={cart._id} time={cart.time} oldAmount={cart.amount} discount={cart.discount} />
                    })}
                </div>
            }
            {cartData.length === 0 &&
